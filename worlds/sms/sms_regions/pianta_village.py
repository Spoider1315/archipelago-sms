...
PIANTA_VILLAGE_ONE: SmsRegion = SmsRegion(SmsRegionName.PIANTA_ONE,
...
PIANTA_VILLAGE_TWO: SmsRegion = SmsRegion(SmsRegionName.PIANTA_TWO,
    requirements=[Requirements(location=f"{SmsRegionName.PIANTA_ONE} - Chain Chomplets Unchained")],
    shines=[Shine("Il Piantissimo's Crazy Climb", in_game_bit=65)],
...
PIANTA_VILLAGE_THREE: SmsRegion = SmsRegion(SmsRegionName.PIANTA_THREE,
    requirements=[Requirements(location=f"{SmsRegionName.PIANTA_TWO} - Il Piantissimo's Crazy Climb")],
    shines=[Shine("The Goopy Inferno", [Requirements([[NozzleType.hover]])], in_game_bit=62),],
...
PIANTA_VILLAGE_FOUR: SmsRegion = SmsRegion(SmsRegionName.PIANTA_FOUR,
    requirements=[Requirements(location=f"{SmsRegionName.PIANTA_THREE} - The Goopy Inferno")],
    shines=[Shine("Chain Chomp's Bath", [Requirements(SPRAY_OR_HOVER)], in_game_bit=61),],
    blue_coins=[],
    parent_region=SmsRegionName.PIANTA_ENTRANCE
)

PIANTA_VILLAGE_FIVE: SmsRegion = SmsRegion(SmsRegionName.PIANTA_FIVE,
    requirements=[Requirements(location=f"{SmsRegionName.PIANTA_FOUR} - Chain Chomp's Bath")],
    shines=[Shine("Secret of the Village Underside", [Requirements([[NozzleType.yoshi]])], in_game_bit=64),
        Shine("Red Coin Chucksters", [Requirements(YOSHI_AND_HOVER)], in_game_bit=68),
...
PIANTA_VILLAGE_SIX: SmsRegion = SmsRegion(SmsRegionName.PIANTA_SIX,
    requirements=[Requirements(location=f"{SmsRegionName.PIANTA_FIVE} - Secret of the Village Underside")],
    shines=[Shine("Piantas in Need", [Requirements(SPRAY_OR_HOVER)], in_game_bit=63),],
...
PIANTA_VILLAGE_SEVEN: SmsRegion = SmsRegion(SmsRegionName.PIANTA_SEVEN,
    requirements=[Requirements(location=f"{SmsRegionName.PIANTA_SIX} - Piantas in Need")],
    shines=[Shine("Shadow Mario Runs Wild", [Requirements([[NozzleType.spray]])], in_game_bit=66)],
    blue_coins=[],
    parent_region=SmsRegionName.PIANTA_ENTRANCE
)

PIANTA_VILLAGE_EIGHT: SmsRegion = SmsRegion(SmsRegionName.PIANTA_EIGHT,
    requirements=[Requirements(location=f"{SmsRegionName.PIANTA_SEVEN} - Shadow Mario Runs Wild")],
    shines=[Shine("Fluff Festival Coin Hunt", [Requirements(ROCKET_OR_HOVER)], in_game_bit=67),
        Shine("Soak the Sun", [Requirements(SPRAY_AND_HOVER_OR_ROCKET)], in_game_bit=69)],
